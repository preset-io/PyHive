...
class TestPresto(unittest.TestCase, DBAPITestCase):
    ...
    def test_escape_args(self):
        ...
        self.assertEqual(
            escaper.escape_args((datetime.date(2020, 4, 17),)), ("date '2020-04-17'",)
        )
        self.assertEqual(
            escaper.escape_args((datetime.datetime(2020, 4, 17, 12, 0, 0, 123456),)),
    ...
    def test_description(self, cursor):
        ...
        self.assertEqual(
            cursor.description, [("foobar", "integer", None, None, None, None, True)]
        )
    ...
    def test_poll(self, cursor):
        ...
        def fail(*args, **kwargs):
            self.fail(
                "Should not need requests.get after done polling"
            )  # pragma: no cover
    ...
    def test_invalid_protocol_config(self):
        ...
        self.assertRaisesRegexp(
            ValueError,
            "Protocol.*https.*password",
            lambda: presto.connect(
    ...
    def test_requests_jwt(self):
        ...
        connection = presto.connect(
            ...
            requests_kwargs={"jwt": "my_jwt"},
        )
        with mock.patch("pyhive.presto.getpass.getuser", return_value="alice"):
            cursor = connection.cursor()
            cursor.execute("SELECT * FROM one_row")

        session.post.assert_called_with(
            "http://localhost:8080/v1/statement",
            auth=presto.JWTAuth("my_jwt"),
            data=b"SELECT * FROM one_row",
            headers={
                "X-Presto-Catalog": "hive",
                "X-Presto-Schema": "default",
                "X-Presto-Source": "pyhive.tests.test_presto.TestPresto.test_requests_jwt",
                "X-Presto-User": "alice",
            },
        )
