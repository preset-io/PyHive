...
class TestSqlAlchemyPresto(unittest.TestCase, SqlAlchemyTestCase):
    def create_engine(self):
        return create_engine(
            "presto://localhost:8080/hive/default?source={}".format(self.id())
        )
    ...
    def test_url_default(self):
        ...
        try:
            with contextlib.closing(engine.connect()) as connection:
                self.assertEqual(
                    connection.execute(
                        text("SELECT 1 AS foobar FROM one_row")
                    ).scalar(),
                    1,
                )
        finally:
    ...
    def test_reserved_words(self, engine, connection):
        ...
        query = str(
            fake_table.select()
            .where(fake_table.c.current_timestamp == "a")
            .compile(engine)
        )
    ...
    def test_jwt(self):
        ...
        with mock.patch("pyhive.presto.getpass.getuser", return_value="alice"):
            with contextlib.closing(engine.connect()) as connection:
                connection.execute(text("SELECT 1 AS foobar FROM one_row"))

        session.post.assert_called_with(
            "http://localhost:8080/v1/statement",
            auth=presto.JWTAuth("my_jwt"),
            data=b"SELECT 1 AS foobar FROM one_row",
            headers={
                "X-Presto-Catalog": "hive",
                "X-Presto-Schema": "default",
                "X-Presto-Source": "pyhive",
                "X-Presto-User": "alice",
            },
        ),
